...
namespace CGE {
    ...
    CGEFrameRecorder::CGEFrameRecorder() : m_recordThread(nullptr), m_recordImageThread(nullptr),
                                           m_encoder(nullptr), m_offscreenContext(nullptr),
        ...
        m_recordingWork = [&](void *pts) {
            ...
            glReadPixels(0, 0, m_dstSize.width, m_dstSize.height, GL_RGBA, GL_UNSIGNED_BYTE,
                         bufferCache.buffer);
        ...
        if (isRecordingStarted() && !m_isRecordingPaused) {
            ...
            if (m_recordThread != nullptr) {
                ...
                glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                                       m_frameHandler->getBufferTextureID(), 0);

                glViewport(0, 0, m_dstSize.width, m_dstSize.height);
                m_cacheDrawer->drawTexture(m_frameHandler->getTargetTextureID());
                glFinish();
                glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                                       m_frameHandler->getTargetTextureID(), 0);

                if (m_recordThread->isActive() && m_recordThread->totalWorks() != 0)
                    return;

                m_recordThread->run(CGEThreadPool::Work(m_recordingWork, (void*)(uintptr_t)(m_currentPTS)));
            } else {
                auto bufferCache = m_recordImageThread->getData4Write();
    ...
    bool CGEFrameRecorder::startRecording(int fps, const char *filename, const char *rotate, int bitRate) {
        delete m_encoder;
        m_encoder = new CGEVideoEncoderMP4();
        m_encoder->setRecordDataFormat(CGEVideoEncoderMP4::FMT_RGBA8888);
        if (!m_encoder->init(filename, rotate, fps, m_dstSize.width, m_dstSize.height, true, bitRate)) {
            delete m_encoder;
            m_encoder = nullptr;
    ...
    bool CGEFrameRecorder::endRecording(bool shouldSave) {
        ...
        //等待录制结束
        while ((m_recordThread != nullptr && m_recordThread->isActive()) ||
               (m_recordImageThread != nullptr && m_recordImageThread->isActive()))
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
    ...
    void CGEFrameRecorder::_createOffscreenContext() {
        ...
        m_recordThread->run(CGEThreadPool::Work([&](void *) {
            ...
            delete m_offscreenContext;
            m_offscreenContext = CGESharedGLContext::create(sharedContext, m_dstSize.width,
                                                            m_dstSize.height,
    ...
    void CGEFrameRecorder::setGlobalFilter(const char *config, CGETextureLoadFun texLoadFunc,
                                           void *loadArg) {
