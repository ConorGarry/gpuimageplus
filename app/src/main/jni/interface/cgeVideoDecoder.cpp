...
namespace CGE {
    struct CGEVideoDecodeContext {
        CGEVideoDecodeContext() : pFormatCtx(nullptr), pVideoCodecCtx(nullptr),
                                  pAudioCodecCtx(nullptr), pVideoCodec(nullptr),
                                  pAudioCodec(nullptr), pVideoFrame(nullptr),
                                  pVideoFrameRGB(nullptr), pAudioFrame(nullptr),
                                  pVideoStream(nullptr), pAudioStream(nullptr),
                                  videoStreamIndex(-1), audioStreamIndex(-1), pSwrCtx(nullptr) {}
    ...
    CGEVideoDecodeHandler::CGEVideoDecodeHandler() : m_bufferPtr(nullptr), m_width(0), m_height(0),
                                                     m_samplingStyle(ssFastBilinear),
    ...
    bool CGEVideoDecodeHandler::open(const char *filename) {
        ...
        if (m_context->pVideoCodec == nullptr ||
            avcodec_open2(m_context->pVideoCodecCtx, m_context->pVideoCodec, nullptr) < 0) {
            return false; //视频解码失败
        }
        ...
        if (m_context->audioStreamIndex != -1) {
            ...
            if (m_context->pAudioCodec == nullptr ||
                avcodec_open2(m_context->pAudioCodecCtx, m_context->pAudioCodec, nullptr) < 0) {
        ...
        return m_context->pVideoFrame != nullptr &&
               m_context->pAudioFrame != nullptr;// && initFrameRGB();
    }
    ...
    bool CGEVideoDecodeHandler::initFrameRGB() {
        ...
        avpicture_fill((AVPicture *) m_context->pVideoFrameRGB, m_bufferPtr, PIX_FMT_RGBA,
                       m_context->pVideoCodecCtx->width, m_context->pVideoCodecCtx->height);
        return true;
    }
    ...
    CGEFrameTypeNext CGEVideoDecodeHandler::queryNextFrame() {
        ...
        while (av_read_frame(m_context->pFormatCtx, &m_context->packet) >= 0) {
            if (m_context->packet.stream_index == m_context->videoStreamIndex) {
                ret = avcodec_decode_video2(m_context->pVideoCodecCtx, m_context->pVideoFrame,
                                            &gotFrame, &m_context->packet);

                if (gotFrame) {
                    m_currentTimestamp = 1000.0 * (m_context->pVideoFrame->pkt_pts -
                                                   m_context->pVideoStream->start_time) *
                ...
                ret = avcodec_decode_audio4(m_context->pAudioCodecCtx, m_context->pAudioFrame,
                                            &gotFrame, &m_context->packet);
...
// 					size_t unpaddedLinesize = m_context->pAudioFrame->nb_samples * av_get_bytes_per_sample((AVSampleFormat)m_context->pAudioFrame->format);
    ...
    const CGEVideoFrameBufferData *CGEVideoDecodeHandler::getCurrentVideoFrame() {
        ...
        memcpy(m_cachedVideoFrame.linesize, m_context->pVideoFrame->linesize,
               sizeof(m_context->pVideoFrame->linesize));
        memcpy(m_cachedVideoFrame.data, m_context->pVideoFrame->data,
               sizeof(m_context->pVideoFrame->data));
    ...
    const CGEAudioFrameBufferData *CGEVideoDecodeHandler::getCurrentAudioFrame() {
        if (m_context->pSwrCtx == nullptr) {
            if (m_context->pAudioStream->codec->sample_fmt != AV_SAMPLE_FMT_S16) {
                ...
                ret = av_samples_alloc_array_and_samples(&m_context->dstSampleData,
                                                         &m_context->dstSamplesLinesize,
                                                         c->channels, m_context->maxDstNbSamples,
                                                         c->sample_fmt, 0);
        ...
        int ret = swr_convert(m_context->pSwrCtx, m_context->dstSampleData,
                              m_context->dstSamplesSize,
                              (const uint8_t **) m_context->pAudioFrame->data,
                              m_context->pAudioFrame->nb_samples);
    ...
    CGEVideoFrameBufferData CGEVideoDecodeHandler::getCurrentVideoFrameRGB() {
        ...
        SwsContext *img_convert_ctx = sws_getContext(m_context->pVideoCodecCtx->width,
                                                     m_context->pVideoCodecCtx->height,
                                                     m_context->pVideoCodecCtx->pix_fmt,
                                                     m_context->pVideoCodecCtx->width,
                                                     m_context->pVideoCodecCtx->height,
                                                     PIX_FMT_RGBA, m_samplingStyle, nullptr,
                                                     nullptr, nullptr);
        // Convert the image from its native format to RGB
        sws_scale(img_convert_ctx, m_context->pVideoFrame->data, m_context->pVideoFrame->linesize,
                  0, m_height, m_context->pVideoFrameRGB->data,
                  m_context->pVideoFrameRGB->linesize);

        CGEVideoFrameBufferData data;
        data.width = m_width;
        data.height = m_height;
        data.format = (CGEVideoFormat) m_context->pVideoFrameRGB->format;
        memcpy(data.linesize, m_context->pVideoFrameRGB->linesize,
               sizeof(m_context->pVideoFrameRGB->linesize));
    ...
    int CGEVideoDecodeHandler::getAmountOfFrames(int ENCODE_FPS) {
        return ((double) m_context->pFormatCtx->duration) / AV_TIME_BASE * ENCODE_FPS;
    }
    ...
    int CGEVideoDecodeHandler::seekTo(int position) {
        int statusser = avformat_seek_file(m_context->pFormatCtx, -1, INT64_MIN, position, INT64_MAX, 0);
        //int statusser = avformat_seek_file(m_context->pFormatCtx, m_context->pVideoStream, INT64_MIN, 2000, INT64_MAX, 0);
        return statusser;
    }
