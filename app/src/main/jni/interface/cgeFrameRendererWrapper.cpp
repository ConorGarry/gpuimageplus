...
JNIEXPORT void JNICALL Java_org_wysaid_nativePort_CGEFrameRenderer_nativeSetFilterWithConfig
        (JNIEnv *env, jobject obj, jlong addr, jstring config, jboolean shouldClearOlder) {
    ...
    renderer->setFilterWithConfig(configStr, cgeGlobalTextureLoadFunc, shouldClearOlder,
                                  &texLoadArg);
...
JNIEXPORT void JNICALL Java_org_wysaid_nativePort_CGEFrameRenderer_nativeSetFilterIntensity
        (JNIEnv *env, jobject, jlong addr, jint progress) {
    CGEFrameRenderer *renderer = (CGEFrameRenderer *) addr;
    renderer->setFilterIntensity(progress);
}

JNIEXPORT void JNICALL Java_org_wysaid_nativePort_CGEFrameRenderer_nativeSetFilterValueAtIndex
        (JNIEnv *env, jobject, jlong addr, jstring value, jint index) {
    const char *nativeValue = env->GetStringUTFChars(value, 0);

    CGEFrameRenderer *renderer = (CGEFrameRenderer *) addr;
    renderer->setFilterValueAtIndex(env, nativeValue, index);
}
...
Java_org_wysaid_nativePort_CGEFrameRenderer_nativeSetRenderSize(JNIEnv *, jobject, jlong addr,
                                                                jint renderWidth,
                                                                jint renderHeight) {
    CGEFrameRenderer *renderer = (CGEFrameRenderer *) addr;
    renderer->setRenderSize(renderWidth, renderHeight);
}
