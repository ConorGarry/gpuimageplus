...
static AVStream *addStream(AVFormatContext *oc, AVCodec **codec,
                           ...
                           int bitRate = 1650000, int audioSampleRate = 44100, const char *rotate = nullptr) {
    AVCodecContext *c;
    AVStream *st;
    ...
    *codec = avcodec_find_encoder(codec_id);
    ...
    // this rotated the video by x degrees (90 in this case). Next step is to acquire the correct rotation value from the source metadata.
...
namespace CGE {
    ...
    bool CGEVideoEncoderMP4::init(const char *filename, const char *rotate, int fps, int width, int height, bool hasAudio,
        ...
        if (m_context->pOutputFmt->video_codec != AV_CODEC_ID_NONE) {
            m_context->pVideoStream = addStream(m_context->pFormatCtx, &m_context->pVideoCodec,
                                                ...
                                                height, bitRate, audioSampleRate, rotate);
        }
