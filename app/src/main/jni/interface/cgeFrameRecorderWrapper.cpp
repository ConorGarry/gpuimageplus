...
//视频录制相关 API
    ...
    (JNIEnv *env, jobject, jlong addr, jint fps, jstring filename, jstring rotate, jint bitRate) {
        const char *path = env->GetStringUTFChars(filename, 0);
        const char *nativeRotate = env->GetStringUTFChars(rotate, 0);
        CGEFrameRecorder *recorder = (CGEFrameRecorder *) addr;
        bool ret = recorder->startRecording(fps, path, nativeRotate, bitRate);
