...
namespace CGE {
    ...
    void CGEImageHandlerInterface::copyTextureData(void *data, int w, int h, GLuint texID,
    ...
    bool CGEImageHandlerInterface::initImageFBO(const void *data, int w, int h, GLenum channelFmt,
    ...
    CGEImageHandler::CGEImageHandler() : m_bRevertEnabled(false), m_drawer(nullptr),
    ...
    bool CGEImageHandler::initWithRawBufferData(const void *imgData, GLint w, GLint h,
                                                CGEBufferFormat format, bool bEnableReversion) {
        CGE_ENABLE_GLOBAL_GLCONTEXT();
        int channel;
        GLenum dataFmt, channelFmt;
        cgeGetDataAndChannelByFormat(format, &dataFmt, &channelFmt, &channel);
        if (channel == 0) return false;
        char *tmpbuffer = cgeGetScaledBufferInSize(imgData, w, h, channel, cgeGetMaxTextureSize(),
                                                   cgeGetMaxTextureSize());
    ...
    bool
    CGEImageHandler::initWithTexture(GLuint textureID, GLint w, GLint h, CGEBufferFormat format,
    ...
    bool
    CGEImageHandler::copyTexture(GLuint dst, GLuint src, int xOffset, int yOffset, int x, int y,
                                 int w, int h) {
        assert(dst != 0 && src != 0);
        useImageFBO();
    ...
    GLuint CGEImageHandler::copyLastResultTexture(GLuint texID) {
        ...
        if (texID == 0)
            texID = cgeGenTextureWithBuffer(nullptr, m_dstImageSize.width, m_dstImageSize.height,
        ...
        if (!copyTexture(texID, m_bufferTextures[1])) {
            ...
            glFinish();
            glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, m_dstImageSize.width,
                                m_dstImageSize.height);
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                                   m_bufferTextures[0], 0);
        }
...
//		CGE_LOG_INFO("####Finished Processing! Time: %gs \n", float(clock() - t) / CLOCKS_PER_SEC);
    ...
    void CGEImageHandler::peekFilters(std::vector<CGEImageFilterInterfaceAbstract *> *vTrans) {
        __android_log_print(ANDROID_LOG_VERBOSE, APPNAME, "amount of filters is %d",
                            (int) m_vecFilters.size());
        *vTrans = m_vecFilters;
    }
    ...
    bool CGEImageHandler::replaceFilterAtIndex(CGEImageFilterInterfaceAbstract *proc, GLuint index,
                                               bool bDelMem) {
        if (index >= m_vecFilters.size())
            return false;
        std::vector<CGEImageFilterInterfaceAbstract *>::iterator iter =
                m_vecFilters.begin() + index;
        if (bDelMem) {
            CGE_ENABLE_GLOBAL_GLCONTEXT();
            delete *iter;
        }
